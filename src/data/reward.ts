
const BASE = "/pictures/moment-and-reward/Achievements";

export const Reward_data = [
    {
      id: 1101,
      title: "INEX ROBOTICS SCHOOL LEAGUE 2017",
      thumbnail: `${BASE}/INEX ROBOTICS SCHOOL LEAGUE 2018.jpg`,
      text: "การแข่งขันหุ่นยนต์ระดับโรงเรียนเครือข่าย INEX",
      images: [],
      date: "2017",
      awardTitle: "รางวัลชนะเลิศ Sumo Robot Legged Junior",
      winners: ["ทีมนักเรียน EasyKids Robotics"],
    },
    {
      id: 1102,
      title: "WORLD ROBOT GAMES NORTH THAILAND 2018",
      thumbnail: `${BASE}/WORLD ROBOT GAMES NORTH THAILAND 2018.jpg`,
      text: "การแข่งขันหุ่นยนต์ระดับภาคเหนือ World Robot Games",
      images: [],
      date: "2018",
      awardTitle: "รางวัลชนะเลิศ Programmable Line Tracing Senior",
      winners: ["ทีมนักเรียน EasyKids Robotics"],
    },
    {
      id: 1103,
      title: "WORLD ROBOT GAMES 2017 SINGAPORE",
      thumbnail: `${BASE}/WORLD ROBOT GAMES 2017 SINGAPORE.jpg`,
      text: "ชัยชนะครั้งใหญ่ระดับนานาชาติ ณ ประเทศสิงคโปร์",
      images: [],
      date: "2017",
      location: "Singapore",
      subActivities: [
        {
          id: 1,
          title: "ชนะเลิศ iBeam Junior",
          description: "รางวัลชนะเลิศหุ่นยนต์เดินตามเส้น iBeam",
          images: [],
        },
        {
          id: 2,
          title: "ชนะเลิศ iBeam Senior",
          description: "รางวัลชนะเลิศหุ่นยนต์เดินตามเส้น iBeam",
          images: [],
        },
        {
          id: 3,
          title: "ชนะเลิศ iBeam Open",
          description: "รางวัลชนะเลิศหุ่นยนต์เดินตามเส้น iBeam",
          images: [],
        },
        {
          id: 4,
          title: "ชนะเลิศ Sumo Legged Open",
          description: "รางวัลชนะเลิศหุ่นยนต์ซูโม่แบบขาเดิน",
          images: [],
        },
      ],
    },
    {
      id: 1104,
      title: "HALL OF FAME: ABU ROBOCON & MAKERTHON",
      thumbnail: `${BASE}/ABU Robocon Thailand 2005.jpg`,
      text: "รวมประวัติศาสตร์รางวัลชนะเลิศระดับประเทศและเอเชียแปซิฟิก",
      images: [],
      date: "2005-2016",
      subActivities: [
        {
          id: 5,
          title: "ABU Robocon Thailand (2009, 2011)",
          description: "ชนะเลิศระดับประเทศ และ ระดับ Asia-Pacific",
          images: [],
        },
        {
          id: 6,
          title: "Panasonic Award (ABU Japan 2009)",
          description: "รางวัลนวัตกรรมยอดเยี่ยมระดับนานาชาติ",
          images: [],
        },
        {
          id: 7,
          title: "TPA Robocon Thailand (2007, 2011)",
          description: "ชนะเลิศระดับอุดมศึกษา",
          images: [],
        },
        {
          id: 8,
          title: "SEA Makerthon 2016",
          description: "ชนะเลิศ IOT Makerthon For Logistics",
          images: [],
        },
        {
          id: 9,
          title: "ABU Robocon Thailand 2005",
          description: "รองชนะเลิศระดับอุดมศึกษา",
          images: [],
        },
      ],
    },
    {
      id: 1105,
      title: "WORLD ROBOT GAMES 2018 PHILIPPINES",
      thumbnail: `${BASE}/WORLD ROBOT GAMES 2018 PHILIPPINES.jpg`,
      text: "ความสำเร็จระดับนานาชาติ ณ ประเทศฟิลิปปินส์ กวาดรางวัลชนะเลิศ 6 รายการ",
      images: [],
      date: "2018",
      location: "Philippines",
      subActivities: [
        {
          id: 10,
          title: "ชนะเลิศ Sumo 1 Kg Legged Junior",
          description: "หุ่นยนต์ซูโม่แบบขาเดิน รุ่นเยาวชน",
          images: [],
        },
        {
          id: 11,
          title: "ชนะเลิศ Sumo 1 Kg Legged Senior",
          description: "หุ่นยนต์ซูโม่แบบขาเดิน รุ่นอาวุโส",
          images: [],
        },
        {
          id: 12,
          title: "ชนะเลิศ Sumo 1 Kg Legged Open",
          description: "หุ่นยนต์ซูโม่แบบขาเดิน รุ่นทั่วไป",
          images: [],
        },
        {
          id: 13,
          title: "ชนะเลิศ iBeam Junior",
          description: "หุ่นยนต์เดินตามเส้น iBeam รุ่นเยาวชน",
          images: [],
        },
        {
          id: 14,
          title: "ชนะเลิศ iBeam Senior",
          description: "หุ่นยนต์เดินตามเส้น iBeam รุ่นอาวุโส",
          images: [],
        },
        {
          id: 15,
          title: "ชนะเลิศ iBeam Open",
          description: "หุ่นยนต์เดินตามเส้น iBeam รุ่นทั่วไป",
          images: [],
        },
      ],
    },
    {
      id: 1106,
      title: "WORLD ROBOT GAMES THAILAND 2018",
      thumbnail: `${BASE}/WORLD ROBOT GAMES THAILAND 2018.jpg`,
      text: "การแข่งขันหุ่นยนต์ชิงแชมป์ประเทศไทย",
      images: [],
      date: "2018",
      subActivities: [
        {
          id: 16,
          title: "ชนะเลิศ Sumo 3 Kg Open",
          description: "หุ่นยนต์ซูโม่หนัก 3 กก. รุ่นทั่วไป",
          images: [],
        },
        {
          id: 17,
          title: "ชนะเลิศ Sumo 3 Kg",
          description: "หุ่นยนต์ซูโม่หนัก 3 กก.",
          images: [],
        },
      ],
    },
    {
      id: 1108,
      title: "CHIANG MAI ROBOT GAMES 2018",
      thumbnail: `${BASE}/CHIANG MAI ROBOT GAMES 2018.jpg`,
      text: "สนามแข่งขันหุ่นยนต์ระดับจังหวัดเชียงใหม่",
      images: [],
      date: "2018",
      awardTitle: "รางวัลชนะเลิศ Beam Line Tracing Open",
      winners: ["ทีมนักเรียน EasyKids Robotics"],
    },
    {
      id: 1109,
      title: "MAKE X THAILAND 2019 (NORTH REGIONAL)",
      thumbnail: `${BASE}/MAKE X THAILAND 2019 ROBOTICS COMPETITION NORTH, THAILAND.jpeg`,
      text: "การแข่งขันรอบคัดเลือกภาคเหนือ ณ จังหวัดเชียงใหม่",
      images: [],
      date: "2019",
      location: "Chiang Mai, Thailand",
      awardTitle: "รางวัลชนะเลิศ MakeX Starter City Guardian Elementary Group",
      winners: ["ทีมนักเรียน EasyKids Robotics"],
    },
    {
      id: 11091,
      title: "MAKE X THAILAND 2019 CHAMPIONSHIP",
      thumbnail: `${BASE}/MAKE X THAILAND 2019 ROBOTICS COMPETITION CHAMPIONSHIP.jpeg`,
      text: "การแข่งขัน MakeX Thailand รอบชิงชนะเลิศ",
      images: [],
      date: "2019",
      location: "Thailand",
    },
    {
      id: 11092,
      title: "MAKEX ROBOTICS COMPETITION WORLD 2019",
      thumbnail: `${BASE}/MAKEX ROBOTICS COMPETITION WORLD 2019 GUANGZHOU, CHINA.jpeg`,
      text: "การแข่งขัน MakeX ระดับโลก ณ เมืองกวางโจว ประเทศจีน",
      images: [],
      date: "2019",
      location: "Guangzhou, China",
    },
    {
      id: 11093,
      title: "ALL JAPAN ROBOT-SUMO TOURNAMENT 2019",
      thumbnail: `${BASE}/ALL JAPAN ROBOT-SUMO TOURNAMENT 2019 TOKYO, JAPAN.jpeg`,
      text: "การแข่งขันหุ่นยนต์ซูโม่ระดับนานาชาติ ณ กรุงโตเกียว ประเทศญี่ปุ่น",
      images: [],
      date: "2019",
      location: "Tokyo, Japan",
    },
    {
      id: 1110,
      title: "SUMO ROBOT ALL THAILAND FSI 2019",
      thumbnail: `${BASE}/SUMO ROBOT ALL THAILAND FSI 2019.jpeg`,
      text: "กวาดรางวัลใหญ่ในสนามหุ่นยนต์ซูโม่ระดับประเทศ",
      images: [],
      date: "2019",
      subActivities: [
        {
          id: 18,
          title: "ชนะเลิศ MicroBit Sumo 1Kg. Open (AUTO)",
          description: "หุ่นยนต์ซูโม่ควบคุมด้วย MicroBit",
          images: [],
        },
        {
          id: 19,
          title: "ชนะเลิศ Standard Sumo 1Kg. Junior (AUTO)",
          description: "หุ่นยนต์ซูโม่มาตรฐาน รุ่นเยาวชน",
          images: [],
        },
        {
          id: 20,
          title: "ชนะเลิศ Standard Sumo 1Kg. Senior (AUTO)",
          description: "หุ่นยนต์ซูโม่มาตรฐาน รุ่นอาวุโส",
          images: [],
        },
        {
          id: 21,
          title: "ชนะเลิศ Standard Sumo 1Kg. Senior (RC)",
          description: "หุ่นยนต์ซูโม่แบบบังคับมือ รุ่นอาวุโส",
          images: [],
        },
        {
          id: 22,
          title: "รองชนะเลิศอันดับ 1 Standard Sumo",
          description: "รางวัลรองชนะเลิศอันดับ 1",
          images: [],
        },
      ],
    },
    {
      id: 1111,
      title: "WORLD ROBOT GAME 2019 THAILAND (Set A)",
      thumbnail: `${BASE}/WORLD ROBOT GAME 2019 THAILAND.jpeg`,
      text: "ผลงานยอดเยี่ยมจากรายการ World Robot Game Thailand",
      images: [],
      date: "2019",
      subActivities: [
        {
          id: 23,
          title: "ชนะเลิศ Biped Robot Open",
          description: "หุ่นยนต์สองขา รุ่นทั่วไป",
          images: [],
        },
        {
          id: 24,
          title: "ชนะเลิศ Line Tracing Programmable Junior",
          description: "หุ่นยนต์เดินตามเส้นแบบเขียนโปรแกรม รุ่นเยาวชน",
          images: [],
        },
        {
          id: 25,
          title: "รองชนะเลิศอันดับ 1 Rugby Robot 1 kg. Master",
          description: "หุ่นยนต์รักบี้ รุ่นมาสเตอร์",
          images: [],
        },
      ],
    },
    {
      id: 1112,
      title: "WORLD ROBOT GAMES THAILAND 2019 (Set B)",
      thumbnail: `${BASE}/WORLD ROBOT GAMES THAILAND 2019.jpeg`,
      text: "ต่อเนื่องความสำเร็จจากสนาม WRG Thailand",
      images: [],
      date: "2019",
      subActivities: [
        {
          id: 26,
          title: "ชนะเลิศ Line Tracing Programmable Master",
          description: "หุ่นยนต์เดินตามเส้นแบบเขียนโปรแกรม รุ่นมาสเตอร์",
          images: [],
        },
        {
          id: 27,
          title: "ชนะเลิศ Sumo Robot 1 Kg. (RC)",
          description: "หุ่นยนต์ซูโม่แบบบังคับมือ",
          images: [],
        },
      ],
    },
    {
      id: 11094,
      title: "การแข่งขันหุ่นยนต์นักเรียนนักศึกษา ปฏิรูปการศึกษาเชียงใหม่ ครั้งที่ 5",
      thumbnail: `${BASE}/การแข่งขันหุ่นยนต์นักเรียนนักศึกษา งาน ปฏิรูปการศึกษาเชียงใหม่ ครั้งที่ 5.jpeg`,
      text: "การแข่งขันหุ่นยนต์ระดับนักเรียนนักศึกษา จังหวัดเชียงใหม่",
      images: [],
      date: "2019",
      location: "Chiang Mai, Thailand",
    },
    {
      id: 1120,
      title: "North East Robot Competition 2020 (NERC 2020)",
      thumbnail: `${BASE}/North East Robot Competition.jpg`,
      text: "การแข่งขันหุ่นยนต์ซูโม่ระดับประเทศ",
      images: [],
      date: "2020",
      subActivities: [
        {
          id: 42,
          title: "ชนะเลิศ: หุ่นยนต์ซูโม่ 3Kg (Open)",
          description: "นายกันต์ กาญจนพิพัฒน์กุล (โรงเรียนดาราวิทยาลัย)",
          images: [],
        },
        {
          id: 43,
          title: "รองชนะเลิศอันดับ 2: หุ่นยนต์ซูโม่ 1Kg (Senior/Junior)",
          description:
            "นายพรศักดิ์ ลี (ดาราวิทยาลัย) และ เด็กชายณัฐกรณ์ นันทการณ์ (ปรินส์รอยแยลส์วิทยาลัย)",
          images: [],
        },
        {
          id: 44,
          title: "รองชนะเลิศอันดับ 3: หุ่นยนต์ซูโม่ 1Kg (Senior/Junior)",
          description:
            "นายทศวัฒน์ชัย ขอบปี (ดาราวิทยาลัย) และ Master Luka Chanakan Bond (วารีเชียงใหม่)",
          images: [],
        },
      ],
    },
    {
      id: 1113,
      title: "FIRST TECH CHALLENGE THAILAND 2020",
      thumbnail: `${BASE}/FIRST TECH CHALLENGE THAILAND.jpg`,
      text: "การแข่งขันหุ่นยนต์ระดับมาตรฐานสากล ณ จ.เชียงใหม่",
      images: [],
      date: "9-11 ธันวาคม 2020",
      location: "โรงเรียนปรินส์รอยแยลส์วิทยาลัย",
      subActivities: [
        {
          id: 28,
          title: "รองชนะเลิศอันดับ 2 & Design Award",
          description:
            "ทีม EasyKids Robotics: นายดนุพัฒน์ ปานประดิษฐ์, นายกษิดิศ ธนะโรจน์รุ่งเรือง, นายภาวัต พึ่งเดชะ, Master Luka Chanakan Bond, เด็กชายโอลิเวอร์ ภูมิ ฮาร์ริส, เด็กหญิงรมน ทุมวงษ์",
          images: [],
        },
        {
          id: 29,
          title: "รองชนะเลิศอันดับ 2",
          description:
            "ทีม Dara Unlimit: นายทศวัฒน์ชัย ขอบปี, นายนนทกร ชุนศักดิ์, นายปารมี จันทร์กระจ่าง, นายพีรวิชญ์ ลือชา, นายกันต์ กาญจนพิพัฒน์กุล, นายพรศักดิ์ ลี, นายธีรวุฒิ อินถา",
          images: [],
        },
      ],
    },
    {
      id: 1114,
      title: "MakeX CHALLENGE ROBOTICS 2020",
      thumbnail: `${BASE}/MakeX Challenge Robotics Competition.jpg`,
      text: "รางวัลชนะเลิศระดับประเทศ ณ Paradise Park กรุงเทพฯ",
      images: [],
      date: "2020",
      subActivities: [
        {
          id: 30,
          title: "รางวัลชนะเลิศ (Champion)",
          description:
            "ทีม EasyKids: นายกันต์ กาญจนพิพัฒน์กุล, นายทศวัฒน์ ขอบปี, นายนนทกร ชุนศักดิ์, นายปารมี จันทร์กระจ่าง, เด็กชาย Luka Bond, นายดนุพัฒน์ ปานประดิษฐ์",
          images: [],
        },
        {
          id: 31,
          title: "รางวัล Best Engineering Book",
          description:
            "รางวัลสมุดบันทึกวิศวกรรมยอดเยี่ยม ทีม EasyKids Robotics",
          images: [],
        },
      ],
    },
    {
      id: 1115,
      title: "ROBO INNOVATOR CHALLENGE 2020",
      thumbnail: `${BASE}/ROBO INNOVATOR CHALLENGE.jpeg`,
      text: "ความสำเร็จด้าน AI และหุ่นยนต์อัจฉริยะ (Software Park Thailand)",
      images: [],
      date: "2020",
      subActivities: [
        {
          id: 32,
          title: "CHAMPION of Self Driving Car + Logistic",
          description:
            "ทีม EasyKids: เด็กชาย Luka Bond, นายกันต์ กาญจนพิพัฒน์กุล, นายทศวัฒน์ ขอบปี, นายนนทกร ชุนศักดิ์",
          images: [],
        },
        {
          id: 33,
          title: "BEST TECHNIQUE of Self Driving Car + Logistic",
          description: "รางวัลเทคนิคยอดเยี่ยม ทีม EasyKids Robotics",
          images: [],
        },
      ],
    },
    {
      id: 1116,
      title: "THAILAND MICRO:CODING (TmC 2020)",
      thumbnail: `${BASE}/THAILAND MICROCODING CHAMPIONSHIP.jpg`,
      text: "ชิงแชมป์การเขียนโปรแกรมควบคุมหุ่นยนต์ระดับประเทศ",
      images: [],
      date: "2020",
      location: "ZEER Rangsit",
      subActivities: [
        {
          id: 34,
          title: "ชนะเลิศ Programmable Line Tracing : Junior",
          description:
            "การแข่งขันหุ่นยนต์เดินตามเส้นแบบเขียนโปรแกรม รุ่นเยาวชน",
          images: [],
        },
        {
          id: 35,
          title: "รองชนะเลิศอันดับ 1 Ball Fighting Robot : Team Open",
          description: "รางวัลรองชนะเลิศอันดับ 1 รุ่นโอเพ่น",
          images: [],
        },
        {
          id: 36,
          title: "รองชนะเลิศอันดับ 2 Programmable Line Tracing : Mentor",
          description: "รางวัลรองชนะเลิศอันดับ 2 รุ่นที่ปรึกษา/ผู้เชี่ยวชาญ",
          images: [],
        },
      ],
    },
    {
      id: 1119,
      title: "WORLD ROBOT GAMES ONLINE EDITION 2021",
      thumbnail: `${BASE}/WORLD ROBOT GAMES ONLINE EDITION.jpg`,
      text: "ความสำเร็จจากสนามออนไลน์ระดับนานาชาติ",
      images: [],
      date: "2021",
      subActivities: [
        {
          id: 38,
          title: "GOLD: Garbage Collector – Junior",
          description: "รางวัลเหรียญทอง เกมเก็บขยะ รุ่นเยาวชน",
          images: [],
        },
        {
          id: 39,
          title: "SILVER: Garbage Collector – Junior/Senior",
          description: "รางวัลเหรียญเงิน เกมเก็บขยะ รุ่นเยาวชน และรุ่นอาวุโส",
          images: [],
        },
        {
          id: 40,
          title: "SILVER: Innovative – Junior",
          description: "รางวัลเหรียญเงิน ประเภทนวัตกรรม รุ่นเยาวชน",
          images: [],
        },
        {
          id: 41,
          title: "BRONZE: Garbage Collector – Senior",
          description: "รางวัลเหรียญทองแดง เกมเก็บขยะ รุ่นอาวุโส",
          images: [],
        },
      ],
    },
    {
      id: 1118,
      title: "FIRST LEGO LEAGUE THAILAND ONLINE 2021/22",
      thumbnail: `${BASE}/FIRST LEGO LEAGUE THAILAND ONLINE.jpeg`,
      text: "การแข่งขัน FLL Thailand Online",
      images: [],
      date: "2022",
      awardTitle: "Core Values Inspiration Award",
      winners: [
        "Danuphat Panjasawatwong (CMIS)",
        "Samart Limlenglert (CMIS)",
        "Chitha Yungshun Hung (CMIS)",
        "Krittin Owittayakul (CMIS)",
        "Natgosol Wang (CMIS)",
        "Phutin Ittirore (CMIS)",
      ],
    },
    {
      id: 1117,
      title: "ROBO INNOVATOR CHALLENGE 2022",
      thumbnail: `${BASE}/ROBO INNOVATOR CHALLENGE1.jpg`,
      text: "การแข่งขันหุ่นยนต์ด้าน AI โดย Software Park Thailand",
      images: [],
      date: "2022",
      subActivities: [
        {
          id: 37,
          title: "รองชนะเลิศอันดับ 2, Best Design & Best Performance",
          description:
            "ทีมรวมนักเรียน/นักศึกษา: นายกันต์ กาญจนพิพัฒน์กุล (มช.), นายพรศักดิ์ ลี (TNI), นายธีรวุฒิ อินถา (มทร.ล้านนา), นายดนุพัฒน์ ปานประดิษฐ์ (มก.ศรีราชา), นายภัคพล ปรีชาวนา (มงฟอร์ต), นายอารถนนท์ จักรวาล (มงฟอร์ต), นายปภังกร นิรวัชร์สุวรรณ (ปรินส์), นายฬูค่า ชนกันต์ บอนด์ (วารี), นายโอลิเวอร์ ภูมิ ฮาร์ริส (วารี), นางสาวรมน ทุมวงษ์ (วารี), นายมิ่งพิมุขข์ พลเมืองดี (วารี)",
          images: [],
        },
      ],
    },
    {
      id: 1121,
      title: "FIRST® TECH CHALLENGE THAILAND 2022-2023",
      thumbnail: `${BASE}/FIRST TECH CHALLENGE THAILAND.jpg`,
      text: "รางวัลรองชนะเลิศอันดับ 1 ระดับประเทศ",
      images: [],
      date: "2023",
      subActivities: [
        {
          id: 45,
          title: "รางวัลรองชนะเลิศอันดับ 1",
          description:
            "ทีมผู้แข่งขัน: นายชนกันต์ บอนด์ (วารี), นายภัคพล ปรีชาวนา (มงฟอร์ต), นายอรรถนนท์ จักรวาล (มงฟอร์ต), นายณรภรต อุณหจักร (วชิรวิทย์), นายพศิน พวงสมบัติ (สาธิต มช.), นายภีมร์ รัตนประภา (มงฟอร์ต), นายอัทธวีร์ วริศวงศ์โกศล (ปรินส์), นายณัฐกรณ์ นันทการณ์ (ปรินส์), นายวรเมธ วงค์กิติ (ยุพราช), นายธนกฤต เย็นฉ่ำ (มงฟอร์ต) | ที่ปรึกษา: นายกันต์ กาญจนพิพัฒน์กุล (มช.)",
          images: [],
        },
      ],
    },
    {
      id: 1122,
      title: "MAKER ROBOTICS CHALLENGE 2022 (MRC)",
      thumbnail: `${BASE}/MAKER ROBOTICS CHALLENGE.jpg`,
      text: "กวาดรางวัลชนะเลิศหลายรายการ ณ สนาม MRC 2022",
      images: [],
      date: "2022",
      subActivities: [
        {
          id: 46,
          title: "ชนะเลิศ ROBOT FUTSAL (OPEN)",
          description:
            "นายภัคพล ปรีชาวนา, นายดนุพัฒน์ ปานประดิษฐ์, นายอารถนนท์ จักรวาล, นายกันต์ กาญจนพิพัฒน์กุล",
          images: [],
        },
        {
          id: 47,
          title: "ชนะเลิศ MEGA SUMO RC (OPEN)",
          description: "นายภัคพล ปรีชาวนา, นายกันต์ กาญจนพิพัฒน์กุล",
          images: [],
        },
        {
          id: 48,
          title: "ชนะเลิศ STANDARD SUMO RC (SENIOR)",
          description: "นายดนุพัฒน์ ปานประดิษฐ์",
          images: [],
        },
        {
          id: 49,
          title: "รองชนะเลิศอันดับ 1 MINI SUMO (OPEN)",
          description: "นายภัคพล ปรีชาวนา, นายอารถนนท์ จักรวาล",
          images: [],
        },
        {
          id: 50,
          title: "รองชนะเลิศอันดับ 2 STANDARD SUMO RC (SENIOR)",
          description: "นายกันต์ กาญจนพิพัฒน์กุล",
          images: [],
        },
      ],
    },
    {
      id: 1123,
      title: "WRO 2022 : WORLD ROBOT OLYMPIAD",
      thumbnail: `${BASE}/World Robot Olympiad.jpg`,
      text: "การแข่งขันโอลิมปิกหุ่นยนต์ ประจำปี 2022",
      images: [],
      date: "2022",
      subActivities: [
        {
          id: 51,
          title: "รางวัลรองชนะเลิศอันดับ 1",
          description:
            "นายภัคพล ปรีชาวนา (มงฟอร์ต), นายอารถนนท์ จักรวาล (มงฟอร์ต), นายปภังกร นิรวัชร์สุวรรณ (ปรินส์รอยแยลส์วิทยาลัย)",
          images: [],
        },
      ],
    },
    {
      id: 1124,
      title: "WORLD SKILLS THAILAND 2022",
      thumbnail: `${BASE}/WORLD SKILLS THAILAND.jpg`,
      text: "การแข่งขันฝีมือแรงงานแห่งชาติ ครั้งที่ 29 (ระดับภาคเหนือ)",
      images: [],
      date: "2022",
      subActivities: [
        {
          id: 52,
          title: "รางวัลชนะเลิศ เหรียญทอง (สาขาหุ่นยนต์เคลื่อนที่)",
          description:
            "นายชนกันต์ บอนด์ (วารีเชียงใหม่), นายภัคพล ปรีชาวนา (มงฟอร์ตวิทยาลัย) | ที่ปรึกษา: นายกันต์ กาญจนพิพัฒน์กุล (มช.)",
          images: [],
        },
      ],
    },
    {
      id: 1125,
      title: "Tech Challenge Thailand",
      thumbnail: `${BASE}/Tech Challenge Thailand.jpg`,
      text: "การแข่งขัน Tech Challenge ระดับประเทศไทย",
      images: [],
      date: "2022",
    },
]
